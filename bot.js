...
if (!WEBHOOK_URL) {
    console.warn('⚠️ WEBHOOK_URL не настроен, используем polling');
    bot.startPolling();
} else {
    console.log('🌐 Режим вебхука активирован');
}
...
// Маршрут для вебхука Telegram (только если WEBHOOK_URL настроен)
if (WEBHOOK_URL) {
...
app.listen(PORT, () => {
    ...
    console.log(`🌐 Webhook URL: ${WEBHOOK_URL || 'No configurado'}`);
