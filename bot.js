...
// Жестко заданные переменные (рабочее решение)
...
// Эндпоинт для добавления баланса
        ...
        for (const [userId, user] of Object.entries(database.users)) {
            ...
            user.investments.forEach((investment, index) => {
                ...
                if (!investment.notifications.purchase && user.telegramId) {
                    ...
                    sentNotifications.set(notificationKey + '_purchase', Date.now());
                    notificationsSent++;
                    needsSaving = true;
                }
                ...
                if (hoursElapsed >= 2 && hoursElapsed < 2.166 &&
                    ...
                    sentNotifications.set(notificationKey + '_2h', Date.now());
                    notificationsSent++;
                    needsSaving = true;
                }
                ...
                if (isCompleted &&
                    ...
                    sentNotifications.set(notificationKey + '_completed', Date.now());
                    notificationsSent++;
                    needsSaving = true;
                }
            });
